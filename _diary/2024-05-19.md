...
```java
KStream<K,V> left = StreamBuilder.stream(...)
KStream<K,V> right = StreamBuilder.stream(...)

var valueJoiner = (leftV, rightV) -> leftV + ":" + rightV
var joinWindow = JoinWindows.ofTimeDifferenceWithNo
left.join(right, valueJoiner, )
```